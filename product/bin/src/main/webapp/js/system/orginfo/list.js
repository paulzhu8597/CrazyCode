...
$(function() {
	...
	//?	
		if(CommnUtil.isHaveSelectOneCheckbox("shippinginfos")){
...
function search(pagenow, isfromsearch) {
	var data = CommnUtil.normalAjax("/system/orginfo/findByPageofOrginfo.do",
	...
	if ("null" != data && 'undefined' != data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			html = html
					+ "<tr>"
					+ "<td style='text-align: center'><input type='checkbox' name='shippinginfos' value='"+ data[i].id + "@_@" + data[i].orgname + "' /> </td>"
					+ "<td style='text-align: center'>" + data[i].orgname + " </td>" 
					+ "<td style='text-align: center'>"+ data[i].mask + " </td>" 
					+ "</tr>";
		}
		$("#shippinginfos").html(html);
...
function setPageInfo(pagenow) {
	var data = CommnUtil.normalAjax("/system/orginfo/getCount.do","orgname=" + $("#orgname").val(), "json");
	if ("null" != data && 'undefined' != data) {
		var pagerecode = data;
...
function initeditcargo() {
		var idAndName = CommnUtil.getCheckBoxValueOfSelect("shippinginfos");
		var data = CommnUtil.normalAjax("/system/orginfo/getOneOrg.do" , "idAndName="+idAndName , "json");
...
function saveEditCargo(){
	...
	var data = CommnUtil.normalAjax("/system/orginfo/saveEditOrg.do",
...
//?????
	var data = CommnUtil.normalAjax("/system/orginfo/doAddCargo.do",
			"addorgname=" + $("#addorgname").val() + 
			"&mask=" + $("#mask").val() ,
			"json");
	if (CommnUtil.notNull(data) && "ok" == data) {
...
function deleteorg() {
	if(CommnUtil.isHaveSelectOneCheckbox("shippinginfos")){
		if(confirm("确认删除单位吗？")){
			var data = CommnUtil.normalAjax("/system/orginfo/deleteOrg.do", "idAndName="+CommnUtil.getCheckBoxValueOfSelect("shippinginfos"), "json");
			if(CommnUtil.notNull(data) && "ok" == data){
				search(0, "true");
			}
			else{
